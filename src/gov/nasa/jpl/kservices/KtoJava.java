...
//                                                "setAllConstraintsToStale"));
    ...
    public ArrayList< FieldDeclaration >
        ...
        if ( entity == null ) {
            propertyList =
                    new ArrayList< PropertyDecl >( JavaConversions.asJavaCollection( Frontend.getTopLevelProperties( this.model() ) ) );
            // Autogenerate a global parameter for each class not instantiated
            // at the global level if there are no global instantiations.
            // TODO: improve scope resolution and turn this back into
            // allClassNames version
            // for ( String className : allClassNames ) {
            if ( instantiatedClassNames.isEmpty() ) {
                for ( String className : topLevelClassNames ) {
                    //if ( !instantiatedClassNames.contains( className ) ) {
                        String autogeneratedName =
                                autoGenerated + className.replaceAll( "\\W", "_" );
                        ClassData.Param p = new ClassData.Param( autogeneratedName,
                                                                 className, null, null );
        ...
        if ( errorInfo ) {
            ...
            if ( !processStdoutAndStderr ) {
                try {
                    ...
                    // playing with latex generation
                    if ( false && !Utils.isNullOrEmpty( kFileNames ) ) {
